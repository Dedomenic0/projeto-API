...
class LivroController {
  ...
  static async listarLivroPorId (req, res) {
    try {
      ...
      erro instanceof mongoose.Error.CastError ? res.status(400).json({ message: "Dado de pesquisa incorreto"}) : res.status(500).json({ message: `${erro.message} - falha na requisição do livro `});
    }
  }
  ...
  static async listarLivrosPorEditora (req, res) {
    ...
    try {
      ...
      res.status(500).json({ message: `${erro.mensage} - falha na requisição` });
    }
  }
};
